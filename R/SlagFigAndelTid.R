...
FigAndelTid <- function(RegData, valgtVar, datoFra='2013-01-01', datoTil='3000-12-31', 
    ...
    if(hentData == 1){		## Hvis spørring skjer fra R på server. 
...
if (valgtVar == 'InnlInnen4eSymptom') {
  ...
  RegData$TidSymptInnlegg <- as.numeric(
    difftime(as.POSIXlt(RegData$Innleggelsestidspunkt,
                        format = "%Y-%m-%d %H:%M:%S"),
             as.POSIXlt(RegData$Symptomdebut,
                        format = "%Y-%m-%d %H:%M:%S"),
             units='hours'))
...
SlagUtvalg <- SlagLibUtvalg(RegData=RegData, datoFra=datoFra, datoTil=datoTil, minald=minald, 
