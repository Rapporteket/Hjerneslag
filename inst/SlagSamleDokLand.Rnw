...
opts_chunk$set(fig.path='./', fig.align='center', fig.show='hold', out.width='.7\\textwidth', cache=FALSE)
	#out.heigth='.45\\pageheight', #
...
<<'LasteDataOgFunksjoner', echo=FALSE, eval=T>>=
#Må laste inn SlagData og tabellen SykehuskoderSlag. Lokalt gjøres dette utenfor.
library(DBI)
library(RMySQL)
library(yaml)
...
con <- dbConnect( dbDriver("MySQL") ,
	 user=conf$Hjerneslag$user,
	 password=conf$Hjerneslag$pass,
	 dbname=conf$Hjerneslag$name,
	 host=conf$Hjerneslag$host)

query <- "SET NAMES utf8;"
tmp <- dbGetQuery(con, query)

#Tatt bort:      HjerneSlagPROD.`RelatedID` + '0' AS RelatedID,
...
query <- "SELECT
     ...
     HjerneSlagPROD.`PreKalsiumantagonist` + '0' AS PreKalsiumanatgonist,
     ...
     HjerneSlagPROD.`PreMedHoytBT` + '0' AS PreMedikBehHoytBT,
     ...
     cast(HjerneSlagPROD.`Updated` as CHAR(10)) AS Updated
FROM
     HjerneSlagPROD"

SlagData <- dbGetQuery(con, query)

#Tas med kun for å få med alle potensielle sykehus slik at de som evt. har 0 registreringer også blir med.
#querySh <- 'select * FROM SykehuskoderSlag'
#SykehusTab <- dbGetQuery(con, querySh)
dbstop <- dbDisconnect(con)

libkat <- '../'		#del av sti til bibliotekkatalog, før /lib/r/<funksjon.R>
libkatTex <- '../'	
source(paste(libkat, "SlagFigAndelerGrVar.R", sep=''), encoding="UTF-8")
source(paste(libkat, "SlagFigMeanMed.R", sep=''), encoding="UTF-8")
source(paste(libkat, "SlagFigAndeler.R", sep=''), encoding="UTF-8")
source(paste(libkat, "SlagFigAndelerKvalInd.R", sep=''), encoding="UTF-8")
source(paste(libkat, "SlagFigAntStabel.R", sep=''), encoding="UTF-8")
source(paste(libkat, "SlagFigAndelerPrePost.R", sep=''), encoding="UTF-8")

@


<<'VariabeldefOgDatautvalg',echo=FALSE, eval=T>>=
...
SlagData$Symptomdebut <- as.POSIXlt(SlagData$Symptomdebut, format="%Y-%m-%d %H:%M:%S" )
SlagData$TimerSymptomdebutInnlegg <- as.numeric(difftime(SlagData$Innleggelsestidspunkt, SlagData$Symptomdebut,  
		units='hours'))
...
#levels(SlagData$ShNavn) <- unique(c(levels(SlagData$ShNavn), 
#					setdiff(levels(as.factor(SykehusTab$Avdeling)),levels(SlagData$ShNavn))))
SlagData$erMann[SlagData$Kjonn == 'K'] <- 0
SlagData$erMann[SlagData$Kjonn == 'M'] <- 1

#Felles parametre:
#egenavd <- 0	#1-Eget sykehus (standard), 0-Hele landet
#sml <- 0		#Sml med resten av landet eller ikke. (0-ikke sml, 1-sml). Ikke aktiv når hele landet valgt.
enhetsUtvalg <- 0
minald <- 0
maxald <- 130
datoFra <- '2013-01-01'	
...
SlagData <- SlagData[(SlagData$InnDato>=as.POSIXlt(datoFra)) & (SlagData$InnDato<=as.POSIXlt(datoTil)), ] 
...
SlagData$Aar <- as.factor(SlagData$InnDato$year+1900)
N <- dim(SlagData)[1]
tabKjPst <- round(table(SlagData$erMann)/N*100, digits=1)
indFra2014 <- which(SlagData$InnDato>=as.POSIXlt('2014-01-01'))

dum <- FigAndelerKvalInd(RegData=SlagData, datoFra=datoFra, datoTil=datoTil, erMann=erMann, NIHSSinn=NIHSSinn, 
			reshID=reshID, enhetsUtvalg=enhetsUtvalg, libkat=libkat, outfile='FigKvalInd.pdf')
...
#indSortSh <- order(levels(SlagData$ShNavn))	#Tror ikke denne er nødvendig...
...
#Fil med flere år:
PerPasAar <- unique(SlagData[ ,c('ShNavn','Aar', 'PasientId')])#aggregate(SlagData$PasientId, by=SlagData[ ,c('ShNavn', 'Aar','PasientId')], FUN=length)
...
dum <- FigAndeler(RegData=SlagData, valgtVar='Alder', datoFra=datoFra, 
		...
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigAlder.pdf')
@
...
dum <- 	FigAndeler(RegData=SlagData, valgtVar='SivilstatusPre', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, 
...
InnlSlag <- FigAndelerGrVar(RegData=SlagData, valgtVar='InnlSlagenh', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigInnlSlag.pdf')

BehSlagenhet <- FigAndelerGrVar(RegData=SlagData, valgtVar='BehSlagenhet', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigBehSlagenhet.pdf')
		
TrombolyseI63 <- FigAndelerGrVar(RegData=SlagData, valgtVar='TrombolyseI63', datoFra=datoFra, 
		datoTil=datoTil, minald=18, maxald=80, erMann=erMann, diagnose=2, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigTrombolyseI63.pdf')

InnlInnen4eSymptom <- FigAndelerGrVar(RegData=SlagData, valgtVar='InnlInnen4eSymptom', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigInnlInnen4eSymptom.pdf')

SvelgtestUtfort <- FigAndelerGrVar(RegData=SlagData, valgtVar='SvelgtestUtfort', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigSvelgtestUtfort.pdf')
		
UtAntitrombotiskI63 <- FigAndelerGrVar(RegData=SlagData, valgtVar='UtAntitrombotiskI63', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=2, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigUtAntitrombotiskI63.pdf')
		
UtAntikoagI63atrie <- FigAndelerGrVar(RegData=SlagData, valgtVar='UtAntikoagI63atrie', datoFra=datoFra, 
		datoTil=datoTil, minald=18, maxald=80, erMann=erMann, diagnose=2, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigUtAntikoagI63atrie.pdf')

UtBT <- FigAndelerGrVar(RegData=SlagData, valgtVar='UtBT', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigUtBT.pdf')
		
LipidI63u80 <- FigAndelerGrVar(RegData=SlagData, valgtVar='LipidI63u80', datoFra=datoFra, 
		datoTil=datoTil, minald=18, maxald=80, erMann=erMann, diagnose=2, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigLipidI63u80.pdf')
		
AntDagerInnlGjsn <- FigMeanMed(RegData=SlagData, valgtVar='AntDagerInnl', valgtMaal='mean', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigAntDagerInnlGjsn.pdf')
			
AntDagerInnlMed <- FigMeanMed(RegData=SlagData, valgtVar='AntDagerInnl', valgtMaal='Med', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, libkat=libkat, outfile='FigAntDagerInnlMed.pdf')
...
\caption{\label{fig:Innl4sympt} Andel pasienter innlagt innen 4,5 timer etter symptomdebut.}
\end{figure}

\begin{figure}[ht]
\centerline{ \scalebox{\Sexpr{sSh}}{\includegraphics{FigSvelgtestUtfort.pdf}} }
\caption{\label{fig:Svelgtest} Andel pasienter som har fått vurdert og/eller testet 
			svelgfunksjon.}
\end{figure}
...
%		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
%		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile=outfile)
%@
...
dum <- FigPrePost(RegData=SlagData, valgtVar='NIHSSTrombolyse', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg,libkat=libkat, outfile='FigNIHSSTrombolysePP.pdf')

dum <- 	FigAndeler(RegData=SlagData, valgtVar='NIHSSendrTrombolyse', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigNIHSSendrTrombolyse.pdf')
		
dum <- 	FigAndeler(RegData=SlagData, valgtVar='BildediagnostikkHjerne', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigBildediagnostikkHjerne.pdf')
		
dum <- 	FigAndeler(RegData=SlagData, valgtVar='BildediagnostikkEkstrakranKar', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigBildediagEksKar.pdf')

dum <- 	FigAndeler(RegData=SlagData, valgtVar='BildediagnostikkIntraraniell', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigBildediagIntraKar.pdf')

dum <- 	FigAndeler(RegData=SlagData, valgtVar='RegistreringHjerterytme', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigHjerterytme.pdf')

dum <- 	FigAndeler(RegData=SlagData, valgtVar='BildediagnostikkHjerte', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigBildediagnostikkHjerte.pdf')
...
dum <- 	FigAndeler(RegData=SlagData, valgtVar='AntDagerInnl', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigAntDagerInnl.pdf')

dum <- 	FigAndeler(RegData=SlagData, valgtVar='AvdUtskrFraHvilken', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigAvdUtskrFraHvilken.pdf')
		
dum <- 	FigAndeler(RegData=SlagData, valgtVar='UtskrTil', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, innl4t=innl4t, 
		NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, hentData=0, libkat=libkat, outfile='FigUtskrTil.pdf')
...
dum <-	FigPrePost(RegData=SlagData, valgtVar='Boligforh', datoFra=datoFra, datoTil=datoTil, 
	minald=minald, maxald=maxald, reshID=reshID, enhetsUtvalg=enhetsUtvalg,libkat=libkat, 
	outfile='FigBoligforh.pdf')

dum <- 	FigPrePost(RegData=SlagData, valgtVar='Bosituasjon', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg,libkat=libkat, 
		outfile='FigBositPrePost.pdf')

dum <- 	FigPrePost(RegData=SlagData, valgtVar='Toalett', datoFra=datoFra, datoTil=datoTil, 
