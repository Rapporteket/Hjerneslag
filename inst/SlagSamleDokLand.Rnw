...
#BT-senkende
#N_BTsenk <- sum(SlagDataUtv$PreBTsenk == 1) Torunn: kommentert ut og endret
N_BTsenk <- sum(SlagDataUtv$PreMedHoytBT == 1)

#SlagDataUtv$PreBTsenkSum <- rowSums(SlagDataUtv[ ,NavnBTsenk], na.rm=T)Torunn: kommentert ut og endret
#SlagDataUtv$PreBTsenkSum <- rowSums(SlagDataUtv[ ,PreMedHoytBT], na.rm=T)
#Gir verdiene c(5:10,13:17,21:24,29:31,37,38,45), hvorav 8 betyr kun to, 9 kun én, 10 ingen
#indIngen <- which(SlagDataUtv$PreBTsenkSum == 10) Torunn: Kommentert ut linje 525-530
...
MedBehPrePst <- 100*rbind( 
	...
	#'...Komb. alle BT-senkende' = c(length(indAlle)/N_BTsenk, NA),Torunn: kommentert ut linje 549-557
