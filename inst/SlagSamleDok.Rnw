...
N <- dim(SlagData)[1]
tabKjPst <- round(table(SlagData$erMann)[c('0','1')]/N*100, digits=1)
...
Rapporten er utarbeidet \Sexpr{N} av Lena Ringstad Olsen ved SKDE, på bestilling fra ledelsen i Norsk Hjerneslagregister.
...
PerPasAarEget <- unique(SlagDataEget[,c('Aar','PasientGUID')])	#aggregate(SlagDataEget$PasientGUID, by=SlagDataEget[ ,c('Aar','PasientGUID')], FUN=length)
...
AntPasAar <- table(unique(SlagData[ ,c('PasientGUID','Aar')])$Aar)	#table(PerPasAar$Aar)
tabPasAarNEget <- rbind(
		'Eget sykehus' = c(AntPasAarEget, length(unique(SlagDataEget$PasientGUID))),	#length(PerPasEget)),
		'Alle sykehus' = c(AntPasAar, length(unique(SlagData$PasientGUID)))
			)
...
xtable(tabHovedOppfEget, digits= c(0,0,0,1), align=c('l', rep('r', ncol(tabHovedOppfEget))), 
		caption='Antall akuttskjema og andel av disse med oppfølgingsskjema, inkl. døde innen 3 mnd etter sykehusoppholdet.
...
KritSupplPstTab <- xtable(KritSupplPst, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Utførte undersøkelser og tiltak, ', txt, '.'),
