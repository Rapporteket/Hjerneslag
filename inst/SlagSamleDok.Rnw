...
tabKjPst <- round(table(SlagDataHele$erMann)[c('0','1')]/Nhele*100, digits=1)
...
tabKjPstEget <- round(table(SlagDataHeleEget$erMann)[c('0','1')]/dim(SlagDataHeleEget)[1]*100, digits=1)
...
men de fleste tabeller og figurer viser resultatet av registrerte innleggelser i \Sexpr{sisteAar}. 
...
\Sexpr{min(SlagDataHele$InnDato)} til \Sexpr{max(SlagDataHele$InnDato)}.
...
\Sexpr{min(SlagDataHeleEget$InnDato)} til \Sexpr{max(SlagDataHeleEget$InnDato)}.
...
xtable(tabHovedOppfEget, digits= c(0,0,0,1), align=c('l', rep('r', ncol(tabHovedOppfEget))), 
		caption=paste0('Antall akuttskjema ', sisteAar, ', og andel av disse med oppfølgingsskjema, inkl. døde innen 3 mnd etter sykehusoppholdet.
...
Nokkelt <- xtable(Pasientkarakteristika, digits=c(0,1,0,0,0,0), align=c('l', rep('r', 5)), #c('l', 'r','r','r','c','r'),
		caption=paste0('Pasientkarakteristika, ', txt, ', ', sisteAar, '.'),
...
BoligforhTab <- xtable(Boligforhold, digits=c(1,1,1,0), align=c('l', 'r', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Funksjonsnivå og boligforhold før hjerneslaget, ', txt, ', ', sisteAar, '.'), 	
...
StatusAkuttTab <- xtable(StatusAkuttPst, digits=c(0,1,1), align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Status i akuttfasen. Andeler av alle registreringer ', sisteAar, ', ', txt,  ' (N=', 
		dim(SlagDataUtv)[1], ').'), label=paste0("tab:StatusAkutt",k))
...
RisikofaktTab <- xtable(Risikofakt, digits=c(1,1,1,0), align=c('l', 'r', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Risikofaktorer før hjerneslaget, ', txt, ', ', sisteAar, '.'), 
...
MedBehPrePstTab <- xtable(MedBehPrePst, digits=1, align=c('l', 'r', 'r'), 	
		caption=paste0('Medikamentell behandling før hjerneslaget. Innleggelser fra og med 1.januar ' , sisteAar,
...
#Antikoagulasjon
	...
	MedBehPrePstDiagTab <- xtable(MedBehPrePstDiag, digits=1, align=c('l', 'r', 'r'), 	
			caption=paste0('Medikamentell behandling før hjerneslaget for pasienter med ',
				diagnose, txt, '. N=', Nutv, '. (Innleggelser fra og med 1.januar ', sisteAar,'.)'), 	#, N =', Nhoved), 2014
...
xtable(AkuttbehI63, digits=c(0,0,1,0,1), align=c('l', 'r', 'r', 'r', 'r'), 
		caption=paste0('Reperfusjonsbehandling av hjerneinfarkt (I63), ', sisteAar,', ', shtxt, ' og hele landet.'),
...
KritSupplPstTab <- xtable(KritSupplPst, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Utførte undersøkelser og tiltak, ', sisteAar,', ', txt, '.'),
...
MedBehUtPlateAntiPstI63Tab <- xtable(MedBehUtPlateAntiPstI63, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		...
		Diagnose hjerneinfarkt (I63), ', txt, ', N=', Nutv, '. (Innleggelser fra og med 1. januar ', sisteAar, '.)'),	 #2014
...
xtable(MedBehUtDivPst, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Antikoagulasjonsbehandling ved utskriving av pasienter med atrieflimmer som har  
			overlevd hjerneinfarkt (I63), ', sisteAar, '.'), 	#, N =', Nhoved),
...
MedBehUtBTstatPstTab <- xtable(MedBehUtBTstatPst, digits=1, align=c('l', 'r', 'r'), 	
		...
		Alle diagnoser, ', sisteAar,', ',txt, ', N=', Nutv, '.'), 	
