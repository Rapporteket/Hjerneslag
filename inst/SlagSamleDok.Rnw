...
knitr::opts_chunk$set(fig.path='', fig.align='center', fig.show='hold', out.width='.8\\textwidth',cache=FALSE)
@

<<'LasteData', echo=FALSE, eval=T>>=
if (!exists('SlagData')){
  SlagData <- SlagRegDataSQL(datoFra='2013-01-01')
  }
@
...
#UTVALG
SlagDataHele <- SlagData[SlagData$erMann %in% 0:1, ]	#Tar ut registreringer som ikke har kjønn
SlagDataHele <- SlagDataHele[(as.Date(SlagDataHele$InnDato)>=as.Date(datoFraHele)) & (SlagDataHele$InnDato<=as.Date(datoTil)), ] 
SlagDataHele$Aar <- 1900 + strptime(SlagDataHele$Innleggelsestidspunkt, format="%Y")$year
SlagDataHeleEget <- SlagDataHele[which(SlagDataHele$ReshId == reshID), ]
datoFra <- '2017-01-01'
sisteAar <- max(SlagDataHele$Aar) 


SlagData <- SlagDataHele[which(SlagDataHele$InnDato >= as.Date(datoFra)), ]
SlagDataEget <- SlagData[which(SlagDataHeleEget$InnDato >= as.Date(datoFra)), ]
Neget <- dim(SlagDataEget)[1]
Nhele <- dim(SlagDataHele)[1]
Nlandet <- dim(SlagData)[1] 
ind <- list(
      Eget = which(SlagData$ReshId == reshID),
			Alle = 1:Nlandet
						)
...
tabKjPstEget <- round(table(SlagDataEget$erMann)[c('0','1')]/Neget*100, digits=1)
tabKjPst <- round(table(SlagData$erMann)[c('0','1')]/Nlandet*100, digits=1)


dum <- SlagFigAndelerKvalInd(RegData=SlagData, datoFra=datoFra, datoTil=datoTil, erMann=erMann, NIHSSinn=NIHSSinn, 
...
Det totale datagrunnlaget er alle registrerte innleggelser i perioden \Sexpr{datoFraHele} til \Sexpr{datoTil}, 
men de fleste tabeller og figurer viser resultatet av registrerte innleggelser i perioden 
\Sexpr{datoFra} til \Sexpr{datoTil} .
...
Tabellene \ref{tab:AntRegEget} og \ref{tab:AntPasAarEget} gir en oversikt over henholdsvis antall 
hendelser og antall pasienter registrert de siste årene. Tabell \ref{tab:AndelOppfEget} gir en oversikt over 
registreringer av hoved- og oppfølgingsskjema fra og med \Sexpr{min(SlagData$InnDato)} til nå. 
...
tabAvdNEget <- rbind(
	'Eget sykehus' = c(table(SlagDataHeleEget$Aar), dim(SlagDataHeleEget)[1]),
	'Alle sykehus' = c(table(SlagDataHele$Aar), Nhele))
...
#PerPasAar <- aggregate(SlagData$PasientGUID, by=SlagData[ ,c('ShNavn', 'Aar','PasientGUID')], FUN=length)
...
#Eget sykehus
#PerPasEget <- table(as.character(SlagDataEget$PasientGUID))	#aggregate(SlagDataEget$PasientGUID, by=SlagDataEget[ ,c('ShNavn', 'PasientGUID')], FUN=length)
PerPasAarEget <- unique(SlagDataHeleEget[,c('Aar','PasientGUID')])	#aggregate(SlagDataEget$PasientGUID, by=SlagDataEget[ ,c('Aar','PasientGUID')], FUN=length)
AntPasAarEget <- table(PerPasAarEget$Aar)
#AntSlagPrPasEget <- table(PerPasEget)
#PerPas <- table(as.character(SlagData$PasientGUID))	#aggregate(SlagDataEget$PasientGUID, by=SlagDataEget[ ,c('ShNavn', 'PasientGUID')], FUN=length)
#PerPasAar <- aggregate(SlagData$PasientGUID, by=SlagData[ ,c('Aar','PasientGUID')], FUN=length)
AntPasAar <- table(unique(SlagDataHele[ ,c('PasientGUID','Aar')])$Aar)	#table(PerPasAar$Aar)
tabPasAarNEget <- rbind(
		'Eget sykehus' = c(AntPasAarEget, length(unique(SlagDataHeleEget$PasientGUID))),	#length(PerPasEget)),
		'Alle sykehus' = c(AntPasAar, length(unique(SlagDataHele$PasientGUID)))
			)
...
#GML:		union(which(OppfolgUtf==2 & AarsakManglendeOppf==3), which(UtskrTil==10)))]))	#Døde
#		union(which(TidDeath<=98)]))))	#Døde innen 98 dager
...
#Eget sykehus
AntOppfEget <- with(SlagDataEget, length(union(which(OppfolgUtf==1), which(TidDeath<=98))))	#Døde innen 98 dager
#		union(which(OppfolgUtf==2 & AarsakManglendeOppf==3), which(UtskrTil==10)))))	#Døde
AntOppfTot <- with(SlagData, length(union(which(OppfolgUtf==1), which(TidDeath<=98))))	#Døde innen 98 dager
AndelOppfEget <- round(AntOppfEget/Neget*100, 1)
AndelOppfTot <- round(AntOppfTot/dim(SlagData)[1]*100, 1)

tabHovedOppfEget <- rbind(
		'Eget sykehus' = c(Neget, AntOppfEget, AndelOppfEget),
		'Alle sykehus' = c(dim(SlagData)[1], AntOppfTot, AndelOppfTot))
		
#colnames(tabHovedOppfEget)[dim(tabHovedOppfEget)[2]]<- 'Totalt:'
colnames(tabHovedOppfEget) <- c('Akuttskjema', 'Oppfølgingsskjema','Andel oppf.:')
xtable(tabHovedOppfEget, digits= c(0,0,0,1), align=c('l', rep('r', ncol(tabHovedOppfEget))), 
		caption=paste0('Antall akuttskjema fra og med ', datoFra, 'og andel av disse med oppfølgingsskjema, inkl. døde innen 3 mnd etter sykehusoppholdet.
		Andelen oppfølging vil være lavere enn den reelle, siden slag oppstått
		de siste 3 måneder ikke har rukket å få oppfølging.'),
		label='tab:AndelOppfEget')

		#		caption='Antall akuttskjema og andel av disse med oppfølgingsskjema. I andelen oppfølgingsskjema
#		inkluderes også døde innen 3 mnd etter sykehusopholdet (uten mulighet for å få oppfølgingsskjema).
...
%Alle registreringer
For hele landet er det i \Sexpr{sisteAar} gjort registrering av innleggelser i perioden 
\Sexpr{min(SlagData$InnDato)} til \Sexpr{max(SlagDataHele$InnDato)}.
I denne perioden utgjorde kvinner og menn utgjorde hhv. \Sexpr{tabKjPst[1]}\% og \Sexpr{tabKjPst[2]}\% av innleggelsene.
...
TidInnTromb <- na.omit(as.numeric(SlagDataUtv$TidInnleggTrombolyse[which(SlagDataUtv$TidInnleggTrombolyse>0)]))
...
Nokkelt <- xtable(Pasientkarakteristika, digits=c(0,1,0,0,0,0), align=c('l', rep('r', 5)), #c('l', 'r','r','r','c','r'),
		caption=paste0('Pasientkarakteristika, ', txt, ', fra og med', datoFra, '.'),
		label=paste0("tab:Pasientkarakteristika",k))
print(Nokkelt)
}		
...
BoligforhTab <- xtable(Boligforhold, digits=c(1,1,1,0), align=c('l', 'r', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Funksjonsnivå og boligforhold før hjerneslaget, ', txt, ', fra og med', datoFra, '.'), 	
		label=paste0("tab:Boligforh",k))
print(BoligforhTab)
}
...
StatusAkutt <- rbind(
	...
	'Språkproblemer' = c(sum(SlagDataUtv$Spraakproblem==1, na.rm=T),
								sum(SlagDataUtv$Spraakproblem==9, na.rm=T),
								sum(SlagDataUtv$Spraakproblem %in% c(1,2,9), na.rm=T)),	#table(SlagDataUtv$Spraakproblem),
...
StatusAkuttTab <- xtable(StatusAkuttPst, digits=c(0,1,1), align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Status i akuttfasen. Andeler av alle registreringer fra og med', datoFra, ', ', 
		txt,  ' (N=', dim(SlagDataUtv)[1], ').'), label=paste0("tab:StatusAkutt",k))
...
RisikofaktTab <- xtable(Risikofakt, digits=c(1,1,1,0), align=c('l', 'r', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Risikofaktorer før hjerneslaget, ', txt, ', fra og med', datoFra, '.'), 
		#Lipidsenkende behandling er registrert fra 1.januar 2014.'), 	#, N =', Nhoved),
		label=paste0("tab:Risiko",k))
print(RisikofaktTab)
...
#NavnBTsenk <- c('PreDiuretica','PreACEhemmer', 'PreA2Antagonist', 'PreBetablokker', 'PreKalsiumantagonist', 'PreMedHoytBT')
...
#PreMedHoytBT DEFINERES NÅ I PREPROSESSERINGA


for (k in 1:2) {
#SlagDataUtv <- SlagData[switch(utvalg[k], Eget = intersect(ind$Eget, ind$Fra2014), Alle = ind$Fra2014), ]
SlagDataUtv <- SlagData[switch(utvalg[k], Eget = ind$Eget, Alle = 1:dim(SlagData)[1]), ]
...
MedBehPrePst <- 100*rbind( 
	...
	'BT-senkende' = c(N_BTsenk/Nutv, sum(SlagDataUtv$PreMedHoytBT == 9, na.rm=T)/Nutv),
...
#	'...Blodtrykksmedikament' = c(length(intersect(which(SlagDataUtv$PreMedHoytBT==1), ind1kun))/N_BTsenk , NA),
...
MedBehPrePstTab <- xtable(MedBehPrePst, digits=1, align=c('l', 'r', 'r'), 	
		caption=paste0('Medikamentell behandling før hjerneslaget. Innleggelser fra og med ' , datoFra,
'. Alle diagnoser, ', txt, ' (N=',	Nutv, ').'), 	
		label=paste0("tab:MedBehPre",k))
...
#Antikoagulasjon
	...
	MedBehPrePstDiagTab <- xtable(MedBehPrePstDiag, digits=1, align=c('l', 'r', 'r'), 	
			caption=paste0('Medikamentell behandling før hjerneslaget for pasienter med ',
				diagnose, txt, '. N=', Nutv, '. (Innleggelser fra og med ', datoFra,'.)'), 	#, N =', Nhoved), 2014
			label=paste0("tab:MedBehPre", k, 'Diag', d))
...
AntInnTromEget <- length(na.omit(SlagDataI63$TidInnleggTrombolyse[indEget]))
...
AkuttbehI63 <- rbind(
	...
	'Hjernebl m/forverring innen 36t' =c(Hjernebl36timerEget['1'], 
	                                     100*Hjernebl36timerEget['1']/sum(Hjernebl36timerEget),
...
xtable(AkuttbehI63, digits=c(0,0,1,0,1), align=c('l', 'r', 'r', 'r', 'r'), 
		caption=paste0('Reperfusjonsbehandling av hjerneinfarkt (I63), fra og med ', datoFra,', ', 
		shtxt, ' og hele landet.'),
...
#og følgelig ukjent...???. 
sumDiag <- sum(SlagDataUtv$Slagdiagnose %in% 1:3, na.rm=T) #c(1,2,9)
...
#					sum(is.na(SlagData$AkutteFokaleutfallPosBilleddiag)), dim(SlagDataUtv)[1]),
	...
	'Uspes. slag (I64)' = c(sum(SlagDataUtv$Slagdiagnose ==3, na.rm=T), NA,  sumDiag),#c(SlagDiag[3], sum(SlagDiag[-3], na.rm=T), NA), 
...
KritSupplPstTab <- xtable(KritSupplPst, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Utførte undersøkelser og tiltak, fra og med', datoFra,', ', txt, '.'),
		label=paste0("tab:KritSupp",k))
...
SlagDataI63 <- SlagData[intersect(which(SlagData$Slagdiagnose == 2), which(SlagData$UtskrTil != 10)), ]
...
MedBehUtPlateAntiPstI63Tab <- xtable(MedBehUtPlateAntiPstI63, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Platehemmende og antikoagulerende behandling ved utskriving (kun levende). 
		Diagnose hjerneinfarkt (I63), ', txt, ', N=', Nutv, '. (Innleggelser fra og med ', datoFra, '.)'),	 #2014
		label=paste0("tab:MedBehPAUt",k))
...
xtable(MedBehUtDivPst, digits=1, align=c('l', 'r', 'r'), 	#c(0,1,1,1)
		caption=paste0('Antikoagulasjonsbehandling ved utskriving av pasienter med atrieflimmer som har  
			overlevd hjerneinfarkt (I63), fra og med', datoFra, '.'), 	#, N =', Nhoved),
...
MedBehUtBTstatPst <- 100*rbind( 
	'BT-senkende' = table(SlagDataUtv$PostMedikBehHoytBT)[c('1','9')]/Nutv,
...
MedBehUtBTstatPstTab <- xtable(MedBehUtBTstatPst, digits=1, align=c('l', 'r', 'r'), 	
		caption=paste0('Blodtrykkssenkende behandling og statiner ved utskriving (kun levende). 
		Alle diagnoser, fra og med', datoFra,', ',txt, ', N=', Nutv, '.'), 	
		label=paste0("tab:MedBehBTSUt",k))
...
dum <-	SlagFigPrePost(RegData=SlagData, valgtVar='Toalett', datoFra=datoFra, 
		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
		innl4t=innl4t, NIHSSinn=NIHSSinn, reshID=reshID, enhetsUtvalg=enhetsUtvalg, preprosess=preprosess, 
...
%	SlagFigPrePost(RegData=SlagData, valgtVar='Roykestatus', datoFra=datoFra, 
%		datoTil=datoTil, minald=minald, maxald=maxald, erMann=erMann, diagnose=diagnose, 
